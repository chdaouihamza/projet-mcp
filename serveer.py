...
server = Server("elyora-pr-checker")
...
async def list_prompts():
    return [
        Prompt(
            ...
            arguments=[
                PromptArgument(name="pr_number", description="Numéro de la PR", required=True),
                PromptArgument(name="strictness_level", description="standard ou strict", required=False),
            ]
        )
    ]
...
async def get_prompt(name: str, arguments: dict):
    if name == "check-pr-health":
        content = PR_HEALTH_TEMPLATE.format(
            pr_number=arguments.get("pr_number"),
            strictness_level=arguments.get("strictness_level", "standard")
        )
...
if __name__ == "__main__":
    mcp.run()
>>>>>>> ae124d624e5909dade7ddba1c4167068d8296f32
